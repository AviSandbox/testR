...
#Activity 2 Deep insight into patient data(
install.packages(c('cluster','fpc','arules'))
library(cluster)
library(fpc)
library(arules)
...
cat("number of clusters according to optimum average
silhouette width (best separated clusters):", pamk.best$nc,
    "\n")
...
rules_data <- data.frame(as.factor(data$Disease),
                         as.factor(km.out$cluster))
rules <- apriori(rules_data)
inspect(rules)

pamk.best <- pamk(datasub)
cat("number of clusters according to optimum average
    silhouette width (best separated clusters):", pamk.best$nc,
    "\n")
plot(pam(datasub, pamk.best$nc))
  


#Activity 2 deeper insight into patient data
